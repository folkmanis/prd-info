...
export class MaterialsEditComponent implements CanComponentDeactivate {
  ...
  onSave() {
    ...
    if (this.initialValue._id) {
      ...
      return this.materialsService.updateMaterial(update)
        .subscribe(material => this.initialValue = material);
    } else {
      return this.materialsService.insertMaterial(this.form.getRawValue())
        .subscribe(material => {
          this.form.markAsPristine();
          this.router.navigate(['..', material._id], { relativeTo: this.route });
        });
    }

...
export class CustomersApiService {
  ...
  async getAll(params?: Record<string, any>): Promise<Customer[]> {
    return this.transformer.toInstanceAsync(
      Customer,
      this.http.get<Record<string, any>[]>(this.path, new HttpOptions(params).cacheable()),
  ...
  async getOne(idOrName: string, params?: Record<string, any>): Promise<Customer> {
    return this.transformer.toInstanceAsync(
      Customer,
      this.http.get<Record<string, any>>(this.path + idOrName, new HttpOptions(params).cacheable())
    );
  }

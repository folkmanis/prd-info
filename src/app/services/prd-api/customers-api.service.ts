...
import { Customer, customerSchema, CustomerUpdate, NewCustomer } from 'src/app/interfaces';
...
export class CustomersApiService {
  ...
  getAll(params?: Record<string, any>) {
    ...
    return this.#validator.validateArrayAsync(customerSchema, data$);
  }
  ...
  customersResource(params: Signal<Record<string, any>>) {
    return httpResource(() => httpResponseRequest(this.#path, new HttpOptions(params()).cacheable()), {
      defaultValue: [],
      parse: this.#validator.arrayValidatorFn(customerSchema),
      equal: isEqual,
    });
  }

  async getOne(idOrName: string, params?: Record<string, any>): Promise<Customer> {
    return this.#validator.validateAsync(customerSchema, this.#http.get(this.#path + idOrName, new HttpOptions(params).cacheable()));
  }
  ...
  async updateOne(id: string, data: CustomerUpdate): Promise<Customer> {
    ...
    return this.#validator.validate(customerSchema, result);
  }
  ...
  insertOne(customer: NewCustomer, params?: Record<string, any>): Promise<Customer> {
    ...
    return this.#validator.validateAsync(customerSchema, result$);
  }

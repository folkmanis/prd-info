...
export class UsersApiService {
  ...
  getOne(name: string): Observable<User> {
    return this.http
      .get(this.path + name, new HttpOptions().cacheable())
  ...
  updateOne(
    ...
    this.checkDemoMode();
    return this.http
      .patch(this.path + id, data, new HttpOptions(params))
      .pipe(this.transformer.toClass(User));
  }
  ...
  insertOne(data: Partial<User>, params?: Params): Observable<User> {
    this.checkDemoMode();
    return this.http
      .put(this.path, data, new HttpOptions(params))
      .pipe(this.transformer.toClass(User));
  }
  ...
  passwordUpdate(username: string, password: string): Observable<User> {
    this.checkDemoMode();
    return this.http
      .patch(this.path + username + '/password', { password })
      .pipe(this.transformer.toClass(User));
  }

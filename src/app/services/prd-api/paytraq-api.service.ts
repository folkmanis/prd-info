...
export class PaytraqApiService {
  ...
  getSale(id: number): Observable<Pt.PaytraqInvoice> {
    return this.http.get<Pt.PaytraqInvoice>(this.path + 'sale/' + id, new HttpOptions().cacheable());
  }

  postSale(data: Pt.PaytraqInvoice): Observable<Pt.PaytraqNewInvoiceResponse> {
    return this.http.put<Pt.PaytraqNewInvoiceResponse>(this.path + 'sale', { data }, new HttpOptions());
  }
}

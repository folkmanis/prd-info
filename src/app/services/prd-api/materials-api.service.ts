...
export class MaterialsApiService {
  ...
  async getOne(id: string): Promise<Material> {
    return this.transformer
      .toInstanceAsync(
        Material,
        this.http.get<Record<string, any>>(this.path + id, new HttpOptions())
      );
  }
  ...
  async updateOne(id: string, data: Partial<Material>, params?: Record<string, any>): Promise<Material> {
    return this.transformer
      .toInstanceAsync(Material, this.http.patch<Record<string, any>>(
        this.path + id,
        data,
        new HttpOptions(params)
      ));
  ...
  async insertOne(data: Partial<Material>, params?: Record<string, any>): Promise<Material> {
    return this.transformer
      .toInstanceAsync(
        Material,
        this.http.put<Record<string, any>>(
          this.path,
          data,
          new HttpOptions(params)
        ));
  ...
  async validatorData<K extends keyof Material & string>(key: K): Promise<Material[K][]> {
    return firstValueFrom(
      this.http.get<Material[K][]>(this.path + 'validate/' + key, new HttpOptions().cacheable())
    );
  }

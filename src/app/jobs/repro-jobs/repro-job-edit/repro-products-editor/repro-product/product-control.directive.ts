import { DestroyRef, Directive, Injector, Input, OnInit, Self, Signal, computed, effect, inject } from '@angular/core';
...
export class ProductControlDirective implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const customerProducts = toObservable(this.component.productsAvailable, { injector: this.injector });
    const selectedName = controls.name.valueChanges;

    combineLatest([selectedName, customerProducts]).pipe(
      debounceTime(300),

...
export class GmailApiService {
  ...
  getMessage(id: string): Observable<Message> {
    return this.http.get<Record<string, any>>(this.path + 'message/' + id, new HttpOptions().cacheable()).pipe(this.transf.toClass(Message));
  }

  modifyMessage(id: string, messageModify: MessageModifyDto): Observable<Message> {
    return this.http.patch<Record<string, any>>(this.path + 'message/' + id, messageModify, new HttpOptions()).pipe(this.transf.toClass(Message));
  }

  getThreads(query: ThreadsFilterQuery): Observable<Threads> {
    return this.http.get<Record<string, any>>(this.path + 'threads', new HttpOptions(query)).pipe(this.transf.toClass(Threads));
  }

  getThread(id: string): Observable<Thread> {
    return this.http.get<Record<string, any>>(this.path + 'thread/' + id, new HttpOptions()).pipe(this.transf.toClass(Thread));
  }
  ...
  getLabel(id: string): Observable<Label> {
    return this.http.get<Record<string, any>>(this.path + 'label/' + id, new HttpOptions()).pipe(this.transf.toClass(Label));
  }

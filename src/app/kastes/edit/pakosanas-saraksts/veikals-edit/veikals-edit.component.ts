...
import { COLORS, Colors, Kaste, KasteColors, MAX_ITEMS_BOX, Veikals } from 'src/app/kastes/interfaces';
...
export class VeikalsEditComponent {
  ...
  private veikalsValueChanges: Signal<Veikals> = computed(() => {
    ...
    if (veikals) {
      ...
      const kastes = veikals.kastes.map((kaste, idx) => this.updateKaste(
        kaste,
        this.form.getRawValue()[idx])
      );
  ...
  private colorsControlGroup(kaste: KasteColors): ColorsGroup {
    ...
    const colors = COLORS.map(color => ({
      [color]: [
        kaste[color], [Validators.required, Validators.min(0), Validators.max(MAX_ITEMS_BOX)]
      ]
    }));
    const controls: { [key in Colors]: [number, ValidatorFn[]] } = Object.assign({}, ...colors);
    return this.fb.nonNullable.group(controls, { validators: [maxItemsValidator(MAX_ITEMS_BOX)] });
...
function colorTotals(kastes: KasteColors[]): KasteColors {
  const tot: KasteColors = Object.assign({}, ...COLORS.map(col => ({ [col]: 0 })));

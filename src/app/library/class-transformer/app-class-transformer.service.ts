...
export class AppClassTransformerService extends ClassTransformer {
  ...
  toClass<T extends Record<string, any>, V extends Array<any>, U = T[]>(obj: ClassConstructor<T>): OperatorFunction<V, U>;
  toClass<T extends Record<string, any>, V, U = T>(obj: ClassConstructor<T>): OperatorFunction<V, U>;
  toClass<T extends Record<string, any>, V>(obj: ClassConstructor<T>): OperatorFunction<V, T | T[]> {
    return map((data) => this.plainToInstance(obj, data, { exposeDefaultValues: true }));
  }
